package msky.dc.recruitment.auditlogpresenter.testdata

class SampleIdentifiers {
    public static String SAVING_ACCOUNT_TYPE_ID = "1"
    public static String CURRENCY_ACCOUNT_TYPE_ID = "2"

    public static String ANDRZEJ_KOWALSKI_CUSTOMER_ID = "1"
    public static String JAN_NOWAK_CUSTOMER_ID = "2"

    public static String ANDRZEJ_KOWALSKI_CURRENCY_ACCOUNT_TRANSACTION_ID = "100"
    public static String ANDRZEJ_KOWALSKI_SAVING_ACCOUNT_TRANSACTION_ID = "222"

    public static String ANDRZEJ_KOWALSKI_10_UNITS_AMOUNT_TRANSACTION_ID = "333"
    public static String ANDRZEJ_KOWALSKI_100_UNITS_AMOUNT_TRANSACTION_ID = "444"

    public static String JAN_NOWAK_SAVING_ACCOUNT_TRANSACTION_ID = "555"

}
